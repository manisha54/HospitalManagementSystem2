...
def register():
    ...
    confirmpassword_label = Label(screen1, bg='white', fg='#FF7F00', text='Confirm Password',
                                  font=('calibria', 12, 'bold'))
    confirmpassword_label.place(x=500, y=230)
    global username
    global  password
    ...
    global bg6
    bg6 = PhotoImage(file='REGISTER 0.1.png')
...
def login():
    ...
    global bg4
    bg4 = PhotoImage(file='LOGIN 0.1.png')
    img1_label = Label(login_frame, image=bg4)
    login_btn = Button(screen2, text="Login",bg='white',image=bg4, width=180,borderwidth=0, height=80,command=login_verify)
    login_btn.place(x=500, y=260)

def main_screen():
    global screen
    screen = Tk()
    screen.geometry('1350x760+100+0')
    screen.resizable(False, False)
    screen.title('Login | Hospital Management System')
    screen.iconbitmap("hos.ico")
    #image
    global bg
    ...
    #image for login
    global bg1
    bg1 = PhotoImage(file="LOGIN.png")
    label1 = Label(screen, image=bg1)

    #image for register
    global bg2
    bg2 = PhotoImage(file="REGISTER.png")
    label2 = Label(screen, image=bg2)

    # textboxes

    Button(screen, image=bg1, height="80", bg='white', borderwidth=0, width="370", command=login).place(x=500, y=200)
    Button(screen, image=bg2, height="80", bg='white', borderwidth=0, width="370", command=register).place(x=500, y=300)
